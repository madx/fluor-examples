...
export default async function Fluor(selector, fluorFn) {
  ...
  // If no selector is given we attach the component on the body and there is
  // no need to watch for added nodes matching the selector.
  if (!selector) {
    return;
  }

  // Create components for new DOM nodes matching the selector or destroy
  // components not matching their selector anymore.
  const observer = new MutationObserver((mutationList) => {
    for (const mutation of mutationList) {
      ...
      for (const mutatedNode of mutatedNodes) {
        const existingComponents = componentRegistry.get(mutatedNode);
        if (mutatedNode.matches?.(selector)) {
          if (!existingComponents) {
            componentRegistry.set(mutatedNode, new Set([mutatedNode]));
          }
          registerComponent(mutatedNode, fluorFn);
        }
        if (existingComponents && !mutatedNode.matches?.(selector)) {
          for (const component of existingComponents) {
            if (component.$fluorFn === fluorFn) {
              existingComponents.delete(component);
              component.destroy();
            }
          }
        }
  ...
  async function registerComponent(node, fluorFn) {
    ...
    const existingComponents = componentRegistry.get(node);
